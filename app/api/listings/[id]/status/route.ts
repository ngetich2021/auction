import { NextResponse } from "next/server";
import { auth } from "@/lib/auth";
import { prisma } from "@/lib/db";
import { queryStkPushStatus } from "@/lib/mpesa";
import { handleListingCallback } from "@/lib/mpesaCallbacks";

export async function GET(_request: Request, ctx: RouteContext<"/api/listings/[id]/status">) {
  const session = await auth();
  if (!session?.user) {
    return NextResponse.json({ error: "Sign in required." }, { status: 401 });
  }

  const { id } = await ctx.params;
  let listing = await prisma.listing.findUnique({
    where: { id },
    select: { status: true, sellerId: true, title: true, failureReason: true, mpesaCheckoutRequestId: true },
  });

  if (!listing || listing.sellerId !== session.user.id) {
    return NextResponse.json({ error: "Listing not found." }, { status: 404 });
  }

  // The webhook that normally flips this out of PENDING is a passive listener — if it never
  // reached us, actively ask Safaricom for the real outcome instead of leaving the seller
  // staring at "PENDING" for a payment that may have already gone through on their phone.
  if (listing.status === "PENDING" && !listing.failureReason && listing.mpesaCheckoutRequestId) {
    const result = await queryStkPushStatus(listing.mpesaCheckoutRequestId);
    if (result.outcome !== "pending") {
      await handleListingCallback(
        { id, sellerId: listing.sellerId, title: listing.title },
        result.outcome === "succeeded",
        result.outcome === "failed" ? result.resultDesc : undefined,
        undefined
      );
      listing = await prisma.listing.findUnique({
        where: { id },
        select: { status: true, sellerId: true, title: true, failureReason: true, mpesaCheckoutRequestId: true },
      });
    }
  }

  return NextResponse.json({ status: listing!.status, failureReason: listing!.failureReason });
}
